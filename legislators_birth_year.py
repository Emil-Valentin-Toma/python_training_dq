#The trainer said they already created legislators variable, so no file read..
...
for x in list(range(0,len(legislators))):
    legislators[x].append(birth_year[x])
print(legislators[:4])

'''
#transforming the list
of lists in simple list:
legislators = []
for x in legis_list:
    for y in x:
